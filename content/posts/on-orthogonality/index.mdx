...
Now consider taking bets on _uncorrelated_ outcomes. This is where portfolio theory becomes powerful. Let <Math>{"\\mathbf{w} = (w_1, w_2, \\ldots, w_n)^T"}</Math> be your portfolio weights (where <Math>{"\\sum w_i = 1"}</Math>), <Math>{"\\mathbf{r}"}</Math> be the vector of asset returns, and <Math>{"\\Sigma"}</Math> be the covariance matrix.
