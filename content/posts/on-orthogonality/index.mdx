...
1. Their inner product is zero: `⟨u, v⟩ = 0`
...
First, a quick overview on why independent bets are great. Suppose you have a bet with expected value `μ` and variance `σ²`. If you take `n` independent identical bets, the central limit theorem tells us:

- Your total expected value scales linearly: `nμ`
- Your total variance scales linearly: `nσ²`
- But your _standard deviation per unit expectation_ scales as `√n`

This means taking more of the same bet gives you `√n` times better risk-adjusted returns, which is pretty neat.

Now consider taking bets on _uncorrelated_ outcomes. This is where portfolio theory becomes powerful. Let `w = (w₁, w₂, ..., wₙ)ᵀ` be your portfolio weights (where `Σwᵢ = 1`), `r = (r₁, r₂, ..., rₙ)ᵀ` be the vector of asset returns, and `Σ` be the covariance matrix where `Σᵢⱼ = Cov(rᵢ, rⱼ)`.
...
The diagonal terms `wᵢ²σᵢ²` represent each asset's individual risk contribution—these scale with the square of your allocation.

But the off-diagonal terms `2wᵢwⱼρᵢⱼσᵢσⱼ` are where diversification happens. With `n` assets, you have `n` diagonal terms but `n(n-1)/2` off-diagonal terms. As `n` grows large, the off-diagonal correlations dominate. If correlations are low, adding more assets drives portfolio variance toward zero even while maintaining positive expected returns. You get the numerator (returns) without the denominator (risk).

The critical insight is in the off-diagonal terms: `2wᵢwⱼρᵢⱼσᵢσⱼ`. When assets are _orthogonal_ (`ρ = 0`), these terms vanish entirely—you get pure variance reduction with no interference. But when `ρ > 0`, these positive cross-terms _add_ to your total variance, contaminating the diversification benefit.

The mathematical beauty: with `n` assets, you have `n` variance terms but `n(n-1)/2` correlation terms. As `n` grows, the correlations increasingly dominate the portfolio variance. If those correlations are near zero (orthogonal dimensions), your portfolio variance shrinks as `σ̄²/n`—you get the law of large numbers working for you. But if `ρ` is high across all pairs, you're essentially holding one asset in `n` different wrappers.

Orthogonality isn't just helpful—it's the _only_ thing that makes diversification work. Without it, `wᵀΣw` doesn't shrink as you add dimensions; it stays stubbornly high because the covariance matrix remains rank-deficient in the effective dimension space.
