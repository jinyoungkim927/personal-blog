...
1. Given input sequence of length <Math>{"n"}</Math>, want ordered sequence of length <Math>{"i"}</Math>
2. Sequence of length <Math>{"n"}</Math>, but you need to sort into the right order then you get <Math>{"i"}</Math> length sequence
